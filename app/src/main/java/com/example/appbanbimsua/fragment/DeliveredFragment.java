...
public class DeliveredFragment extends Fragment {
    ...
    private void fetchOrders() {
        showProgress();
        Utils.fetchOrders(getContext(), Utils.getUserInfo(getContext()).getId(), 3, new Utils.OrderCallback() {
            @Override
            public void onSuccess(List<OrderList> orders) {
                progressDialog.dismiss();
                if (orders != null) {
                    orderAdapter = new ListOrderAdapter(getContext(),orders,DeliveredFragment.this);
